...
class EWDNN(nn.Module):
    ...
    def forward(self, X):
        ...
        Final Output
        """
        final_output = self.act_final(self.final_out(dnn_input)).squeeze()
            # (batch_size)
        return final_output
...
class FM(nn.Module):
    ...
    def forward(self, X):
        ...
        """
            sum
        """
        total_sum = self.act_final(torch.sum(linear_out, 1) + torch.sum(fm_second_order, 1))
        return total_sum
...
class WideAndDeep(nn.Module):
    ...
    def forward(self, X):
        ...
        """
            sum
        """
        total_sum = self.act_final(torch.sum(linear_out, 1) + torch.sum(dnn_out, 1))
        return total_sum
...
class DeepFM(nn.Module):
    ...
    def forward(self, X):
        ...
        """
            sum
        """
        total_sum = self.act_final(torch.sum(linear_out, 1) + torch.sum(fm_second_order, 1) + torch.sum(dnn_out, 1)) # (bs)
        return total_sum
    
    

class DNN(nn.Module):
    def __init__(self, input_dim, hidden_dim, hidden_num, dropout, l2_reg=0, use_bn=False, init_std=0.0001):
...
class AutoIntNet(nn.Module):
    def __init__(self, trial, size_tuple_list, head_num=4, attn_layers=3, scaling=True, use_residual=True, use_bn=True):
    ...
    def forward(self, X):
        ...
        # final prediction
        outs = self.act_final(self.final(dnn_out)) # (batch_size, 1)
...
class DCN(nn.Module):
    def __init__(self, trial, size_tuple_list, 
                cross_num=3, cross_param='vector', l2_reg=0.00001, mode="train"):
        ...
        # Output layer
        self.bn = nn.BatchNorm1d(self.dnn_input_dim + self.hidden_dims)
        self.final = nn.Linear(self.dnn_input_dim + self.hidden_dims, 1, bias=True)
        self.act_final = nn.Hardtanh(min_val=0, max_val=3)
        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_normal_(m.weight, gain=nn.init.calculate_gain('tanh'))
    ...
    def forward(self, X):
        ...
        stack_out = self.bn(stack_out)
        outs = self.act_final(self.final(stack_out)).squeeze(1) # (batch_size)
        # outs = self.act_final(self.final(dnn_out)) # (batch_size, 1)
        return outs