...
def GetEPSPred(i, rank, data, models, directions, price_df, index_df, strategy='TopKAve', days=251):
    ...
    return tar_report, stock_roi, stock_ar, stock_croi, stock_car
...
def FilterTarReport(tar_report, stock_croi, stock_car):
    temp_tar_report = tar_report[(tar_report['PE'] < 50), :]
    temp_stock_croi = stock_croi[(tar_report['PE'] < 50), :]
    temp_stock_car = stock_car[(tar_report['PE'] < 50), :]
    # 重置index
    temp_tar_report.index = range(len(temp_tar_report))
    return temp_tar_report, temp_stock_croi, temp_stock_car



def ReturnValidation(tar_report, stock_croi, stock_car, models, top_rate=0.01, days=251):
    ...
    return average_croi_df, average_car_df